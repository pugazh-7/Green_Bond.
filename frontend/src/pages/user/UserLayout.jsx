    ...
    return (
        <div className="flex h-screen bg-gray-50 overflow-hidden">
            ...
            <div className="flex flex-col flex-1 overflow-hidden relative bg-gray-50/50">
                ...
                {/* Mobile Top Bar (with Location Header) */}
                <div className="md:hidden flex items-center justify-between p-3 bg-white border-b border-gray-100 z-30">
                    <div className="flex items-center gap-2">
                        <button onClick={() => setIsSidebarOpen(true)} className="p-1.5 -ml-1.5 text-gray-500 hover:bg-gray-50 rounded-lg">
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                        </button>
                        <LocationHeader />
                    </div>
                    <img src={logo} alt="GreenBond" className="w-8 h-8 object-contain rounded-md" />
                </div>

                {/* Desktop Top Bar (with Location Header) */}
                <div className="hidden md:flex items-center justify-between px-8 py-4 bg-white border-b border-gray-100 z-30 shadow-sm">
                    <LocationHeader />
                    <div className="flex items-center gap-4">
                        {/* Placeholder for future desktop header items (e.g. search, profile snippet) */}
                    </div>
                </div>
                ...
                {cartCount > 0 && location.pathname !== '/user/cart' && !location.pathname.includes('/checkout') && (
                    <div className="md:hidden fixed bottom-[84px] left-4 right-4 z-40 animate-slide-up">
                        <Link 
                            to="/user/cart" 
                            className="bg-green-700 text-white rounded-2xl p-4 flex items-center justify-between shadow-lg shadow-green-700/20 active-press"
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    {/* We can fetch and display total price here later, hardcoded to view cart for now */}
                                    <span className="text-xs text-green-100 font-medium">View cart to checkout</span>
                                </div>
                            </div>
